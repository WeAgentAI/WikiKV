...
def run_once(db, prefix):
    start = time.perf_counter()
    rows = 0
    for key, _ in db.iterator(prefix=prefix.encode("utf-8")):
        if key.startswith(b"__"):
            continue
        if key.decode("utf-8", errors="ignore").endswith(".md"):
            rows += 1
...
def main():
    ...
    parser.add_argument("--output", default="/data/wiki/log/leveldb_q4_prefix_search_times.csv")
    ...
    try:
        items = [run_once(db, prefix) for prefix in sampled]
    finally:
        db.close()
    ...
    print("query_type=LEVELDB_Q4_PREFIX_SEARCH")
